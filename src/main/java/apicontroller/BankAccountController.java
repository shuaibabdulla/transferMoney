...
public class BankAccountController {
    ...
    public static Handler createAccount =  ctx ->{
        ...
        boolean result = BankAccountDAO.getInstance().addAccount(newAccount);
        if(!result){
    ...
    public static Handler deleteAccount = ctx -> {
        ...
        boolean res = BankAccountDAO.getInstance().removeAccount(accountID);
        if(!res){
    ...
    public static Handler getAccount = ctx -> {
        String accountID = ctx.pathParam("id");
        Account retrievedAccount = BankAccountDAO.getInstance().getAccount(accountID);
