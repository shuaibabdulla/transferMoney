...
public class TestAccountGet extends ApiTestSuite{
    ...
    public void testAccountGetShouldSuccess() throws UnirestException {
        String testJson = "{\n" +
                ...
                "\"balance\" : 200\n" +
                "}";
        HttpResponse<JsonNode> jsonResponseCreate = Unirest.post( TestConstants.BASE_URL+ TestConstants.ACCOUNT_PATH)
                .header("accept", "application/json")
                .body(testJson)
                .asJson();
        Assert.assertEquals(jsonResponseCreate.getStatus(), HTTPCodes.CREATED.getCode());

        HttpResponse<JsonNode> jsonResponseGet =
                Unirest.get( TestConstants.BASE_URL+ TestConstants.ACCOUNT_PATH + "/" + TestConstants.ACCOUNT_ID_FOR_GET)
                        .header("accept", "application/json")
                        .asJson();
        Assert.assertEquals(jsonResponseGet.getStatus(), HTTPCodes.SUCCESS.getCode());
    }
